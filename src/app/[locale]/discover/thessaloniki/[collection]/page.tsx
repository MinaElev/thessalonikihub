...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    getCollections("discover").map((c) => ({ locale, collection: c.slug })),
  );
}
...
export async function generateMetadata({
  ...
  return collectionMetadata("discover", collection, locale);
}
...
export default async function Page({
  ...
  if (!getCollection("discover", collection)) notFound();
  return <CollectionListing pillar="discover" slug={collection} locale={locale} />;
}
