...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    getCollections("drink").map((c) => ({ locale, collection: c.slug })),
  );
}
...
export async function generateMetadata({
  ...
  return collectionMetadata("drink", collection, locale);
}
...
export default async function Page({
  ...
  if (!getCollection("drink", collection)) notFound();
  return <CollectionListing pillar="drink" slug={collection} locale={locale} />;
}
